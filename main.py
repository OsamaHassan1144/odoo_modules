...
class HomeStaticTemplateHelpers(object):
    ...
    def _get_parent_template(self, addon, template):
        ...
        parent_addon, parent_name = tuple(split_name_attempt) if len(split_name_attempt) == 2 else (
            addon, original_template_name)
        if parent_addon not in self.template_dict:
            if original_template_name in self.template_dict[addon]:
                ...
                raise ValueError(
                    _('Module %s not loaded or inexistent, or templates of addon being loaded (%s) are misordered') % (
                        parent_addon, addon))

        if parent_name not in self.template_dict[parent_addon]:
            raise ValueError(
                _("No template found to inherit from. Module %s and template name %s") % (parent_addon, parent_name))
        ...
        for template_tree in list(all_templates_tree):
            ...
            if self.STATIC_INHERIT_DIRECTIVE in template_tree.attrib:
                ...
                if inherit_mode == self.PRIMARY_MODE:  # New template_tree: A' = B(A)
                    ...
                    inherited_template.set(self.STATIC_INHERIT_DIRECTIVE,
                                           template_tree.attrib[self.STATIC_INHERIT_DIRECTIVE])
                    if self.debug:
...
class GroupsTreeNode:
    ...
    def _get_aggregate(self, field_name, data, group_operator):
        ...
        if not aggregate_func:
            _logger.warning("Unsupported export of group_operator '%s' for field %s on model %s" % (
                group_operator, field_name, self._model._name))
            return
...
class Session(http.Controller):
    ...
    ## custom pos api mod
    @http.route('/api/user/authenticate', type='json', auth="none", cors="*")
    def user_authenticate(self, email, password, db=False, api_type=False, req_arg=False,
                          base_location=None):
        if not db:
            ...
            query = """Select database FROM login_data WHERE email = '""" + email + """';"""
            ...
            if data_base:
                ...
                var_dict = {}
                var_list = []
                # ------ returning pos session information -------
                ...
                if pos_info:
                    ...
                    var_list.append(db_obj)
                    for x in pos_info:
                        var_dict = {
                            'id': x.id,
                            'name': x.name,
                            'company_name': x.company_id.name,
                            'user_name': x.write_uid.name,
                            'pos_type': x.pos_type_sel
                        }
                ...
                return False
        request.session.authenticate(db, email, password)
        if api_type == 'floor_table_info':
            if 'config_id' in req_arg:
                # ------ returning pos session information -------
                pos_info = http.request.env['pos.config'].search([('id', '=', req_arg['config_id'])])
                        ...
                        if tables:
                            ...
                            for x in tables:
                                tables_dict = {
                                    'name': x.name,
                                    'seats': x.seats,
                                    'multiple_order': x.multiple_order,
                                }
                                tables_list.append(tables_dict)
        ...
        elif api_type == 'instructions':
            # ------ returning all product instructions -------
            instructions = []
            instruction_info = {}
            ...
            return instructions
        elif api_type == 'instruction_info':
            if 'instruction_id' in req_arg:
                instructions = []
                instruction_info = {}
                instruct_data = http.request.env['pos.message'].search([('id', '=', req_arg['instruction_id'])])
                if instruct_data:
                    for x in instruct_data:
                        instruction_info = {
                            'id': x.id,
                ...
                return "Invalid Parameter !"
        elif api_type == 'salesmen':
            # ------ returning all salesmen information -------
            salesman = []
            salesman_info = {}
            ...
            return salesman
        elif api_type == 'saleman_info':
            if 'salesman_id' in req_arg:
                print("acquiring salesman for your pos")
                salesman = []
                salesman_info = {}
                salesmen_data = http.request.env['pos.workers.salesmen'].search([('id', '=', req_arg['salesman_id'])])
                if salesmen_data:
                    for x in salesmen_data:
                        salesman_info = {
                            'id': x.id,
                ...
                return "Invalid Parameter !"
        elif api_type == 'order_history':
            if 'customer_name' in req_arg:
                # ------ returning order history -------
                res = http.request.env['pos.order'].search([('partner_id.name', '=', req_arg['customer_name'])])
                list_order = []
                my_id = 1
                my_item = ""
                for x in res:
            ...
            return cars
        elif api_type == 'car_info':
            if 'car_id' in req_arg:

                cars = []
                car_info = {}
                cars_data = http.request.env['user.cars'].search([('id', '=', req_arg['car_id'])])
                if cars_data:
                    for x in cars_data:
                        car_info = {
                            'car_brand': x.car_brand.id,
            ...
            return car_brands
        elif api_type == 'car_brand_info':
            if 'brand_id' in req_arg:
                car_brands = []
                brand_info = {}
                brands_data = http.request.env['user.cars.brands'].search([('id', '=', req_arg['brand_id'])])
                if brands_data:
                    for x in brands_data:
                        brand_info = {
                            'id': x.id,
                ...
                return "Invalid Parameter !"
        elif api_type == 'car_reading_info':
            if 'car_id' in req_arg:
                car_reading_info = {}
                car_reading_data = http.request.env['user.cars.readings'].search([('id', '=', req_arg['car_id'])])
                if car_reading_data:
                ...
                return car_reading_info
            else:
                return "Invalid Parameter !"
        elif api_type == 'invoice_search':
            if 'order_number' in req_arg:
                res = http.request.env['pos.order'].search([('pos_reference', 'like', req_arg['order_number'])])
                list_order = []
                dict_order = {}
                list_item = []
                dict_item = {}
                if res:
                    for x in res:
                        ...
                        date_time_str_d = date_time_str.strftime("%m/%d/%Y")
                        date_time_str_t = date_time_str.strftime("%H:%M")
                        d_t_str = date_time_str_d + " " + date_time_str_t
                        for y in x.lines:
                            dict_item = {
                                'product_name': y.display_name,
                                'qty': y.qty,
                                'rate': y.price_subtotal,
                                'discount': y.discount,
                                'price_subtotal_include_tax': y.price_subtotal_incl,
                            }
                            list_item.append(dict_item)
                        dict_order = {
                            'date_time': d_t_str,
                            'order': x.pos_reference,
                            'table_name': x.table_id.display_name,
                            'customer_name': x.partner_id.name,
                            'customer_mobile': x.partner_id.mobile,
                            'customer_phone': x.partner_id.phone,
                            'item_list': list_item,
                            'total_amount': x.amount_total - x.amount_tax,
                            'total_tax': x.amount_tax,
                            'payable_amount': x.amount_total,
                            'paid_amount': x.amount_paid,
                            'car_number': x.car_id.vehicle_no + " " + x.car_id.car_model,
                            'km_per_day': x.car_id.car_reading_per_day,
                            'current_reading': x.car_id.current_reading,
                            'km_for_next_service': x.car_id.km_for_next_service,
                            'next_service_date': x.car_id.km_for_next_service,
                        }
                return dict_order
            else:
                return "Invalid Parameter !"
        elif api_type == 'create_car':
            if 'car_info' in req_arg:
                print('creating car')
                res = request.env['user.cars'].create_car(req_arg['car_info'])
                print(res)
            else:
                return "Invalid Parameter !"
        elif api_type == 'create_customer':
            if 'customer_info' in req_arg:
                print('creating customer')
                partner_C = request.env['res.partner'].create(req_arg['customer_info'])
                print(partner_C)
            else:
                return "Invalid Parameter !"
        elif api_type == 'create_order':
            if 'order_info' in req_arg:
                print('creating order')
                order_C = request.env['pos.order'].create_from_ui(req_arg['order_info'])
                print(order_C)
            else:
                return "Invalid Parameter !"
        elif api_type == 'customers':
            if 'pos_type' in req_arg:
                print('checking which pos')
                if req_arg['pos_type'] == 'simple':
                    # ------ returning customers normal information -------
                    partners = []
                    customer_info = {}
                    ...
                    return partners
                elif req_arg['pos_type'] == 'drivethru':
                    # ------ returning customers information -------
                    partners = []
                    customer_info = {}
                    cust_data = http.request.env['res.partner'].search([])
                    if cust_data:
                        for x in cust_data:
                            ...
                            partners.append(customer_info)
                    return partners
                elif req_arg['pos_type'] == 'oil_rest_hotel':
                    partners = []
                    customer_info = {}
                    car_info = {}
                    cars = []
            ...
            if all(k in req_arg for k in ('pos_type', 'config_id')):
                print('checking which pos')
                if req_arg == 'simple' or req_arg == 'oil_rest_hotel':
                    # ------ returning all products information -------
                    product_o = []

                    product_info = {}
                    products_pos = http.request.env['product.product'].search([('available_in_pos', '=', True)])
                    if products_pos:
                        for y in products_pos:
                            ...
                            product_o.append(product_info)
                    return product_o
                elif req_arg == 'drivethru':
                    # ------ returning all products information -------
                    products_allow_pos = http.request.env['pos.config'].search([('id', '=', req_arg['conf_id'])])
                    allowed_categories = []
                    products = []
                    product_info = {}
                    ...
                    if products_pos:
                        for y in products_pos:
                            ...
                            if y.product_color:
                                product_color = {
                                    'color': y.product_color.display_name,
                                    'color_code': y.product_color.color_html_code
                                }
                                top_bg_color = {
                                    'color': y.top_bg_color.display_name,
                                    'color_code': y.top_bg_color.color_html_code
                                }
                                top_text_color = {
                                    'color': y.product_color.display_name,
                                    'color_code': y.product_color.color_html_code
                                ...
                                bottom_bg_color = {
                                    'color': y.bottom_bg_color.display_name,
                                    'color_code': y.bottom_bg_color.color_html_code
                                }
                                bottom_text_color = {
                                    'color': y.bottom_text_color.display_name,
                                    'color_code': y.bottom_text_color.color_html_code
                                }
                            ...
                            products_avail = {
                                ...
                                'product_color': product_color or False,
                                'top_bg_color': top_bg_color or False,
                                'top_text_color': top_text_color or False,
                                'bottom_bg_color': bottom_bg_color or False,
                                'bottom_text_color': bottom_text_color or False,
                                'product_tmpl_id': y.product_tmpl_id.id,
                ...
                return "Invalid Parameter !"
        elif api_type == 'product_info':
            if all(k in req_arg for k in ('pos_type', 'product_name')):
                print('checking which pos')
                if req_arg['pos_type'] == 'simple' or req_arg['pos_type'] == 'oil_rest_hotel':
                    product_o = []
                    product_info = {}
                    products_pos = http.request.env['product.product'].search(
                        [('available_in_pos', '=', True), ('name', 'like', req_arg['product_name'])])
                    if products_pos:
                        for y in products_pos:
                    ...
                    if products_pos:
                        for y in products_pos:
                            ...
                            if y.product_color:
                                ...
                                top_text_color = {
                                    'color': y.top_text_color.display_name,
                                    'color_code': y.top_text_color.color_html_code
                                }
                                bottom_bg_color = {
                                    'color': y.bottom_bg_color.display_name,
                                    'color_code': y.bottom_bg_color.color_html_code
                ...
                return "Invalid Parameter !"
        elif api_type == 'customer_info':
            if all(k in req_arg for k in ('pos_type', 'customer_name')):
                print('checking which pos')
                if req_arg['pos_type'] == 'simple':
                    # ------ returning customers normal information -------
                    partners = []
                    customer_info = {}
                    cust_data = http.request.env['res.partner'].search([('name', 'like', req_arg['customer_name'])])
                    if cust_data:
                        for x in cust_data:
                            if x.image_1920:
                                image_base = x.image_1920
                    ...
                    cars = []
                    cust_data = http.request.env['res.partner'].search(
                        [('customer', '=', True), ('name', 'like', req_arg['customer_name'])])
                    if cust_data:
                        for x in cust_data:
                            if x.image_1920:
                                image_base = x.image_1920
                ...
                return "Invalid Parameter !"
        elif api_type == 'config_info':
            if all(k in req_arg for k in ('pos_type', 'config_id')):
                print('checking which pos')
                if req_arg['pos_type'] == 'simple' or req_arg['pos_type'] == 'drivethru':
                    val_list = {}
                    pos_payments = []
                    allowed_categ_arr = []
                    cate_list = {}
                    ...
                    if pos_info:
                        if pos_info.worker_info:
                            for x in pos_info.worker_info.ids:
                                worker_ids.append(x)
                        else:
                        ...
                        val_list = {
                            ...
                            'limit_categories': pos_info.limit_categories,
                            'available_categories': allowed_categ_arr or False,
                            'salesmen_id': worker_ids or False,
                            'is_table_management': pos_info.is_table_management,
                            'ask_password_order_discount': pos_info.ask_password_order_discount,
                            'ask_password_discount': pos_info.ask_password_discount,
                            'ask_fix_password_discount': pos_info.ask_fix_password_discount,
                            'check_disc_fix': pos_info.check_disc_fix,
                            'orderline_notes': pos_info.iface_orderline_notes,
                            'payment_method': pos_payments or False,
                        }
                    else:
                ...
                return "Invalid Parameter !"
        elif api_type == 'save_order':
            print('saving current orders')
            if 'data' in req_arg:
                res = request.env['pos.points'].update_order_status(req_arg['data'])
                print(req_arg['data'])
                return "Order Saved"
            else:
                return "Invalid Parameter !"
        elif api_type == 'update_customer_points':
            print('update customer points')
            if 'data' in req_arg:
                res = request.env['res.partner'].updating_points(req_arg['data'])
                print(req_arg['data'])
                return "Updated points"
            else:
                return "Invalid Parameter !"
        elif api_type == 'kds_order':
            if all(k in req_arg for k in ('o_id', 'data')):
                print('create order KDS points if not exist')
                res = request.env['pos.points'].check_order_db(req_arg['o_id'])
                if res:
                    print('order doesn\'t exist')
                    ses = request.env['pos.points'].create_order(req_arg['data'])
                    return "order created"
                else:
                    print('order exist')
                    tes = request.env['pos.points'].update_order_info(req_arg['data'])
                    return "order updated"
            else:
                return "Invalid Parameter !"
        elif api_type == 'kds_queuelines_create':
            if all(k in req_arg for k in ('o_id', 'data')):
                print('create order\'s  queuelines if not exist')
                res = request.env['pos.points'].check_queue_id(req_arg['o_id'])
                if res:
                    print('order found')
                    ses = request.env['pos.points.line'].create_queue_order(req_arg['data'])
                    return "queuelines created"
                else:
                    return "order not found"
            else:
                return "Invalid Parameter !"
        elif api_type == 'kds_queuelines_update':
            if all(k in req_arg for k in ('o_id', 'data')):
                print('update order\'s  queuelines if  exist')
                res = request.env['pos.points'].check_queue_id(req_arg['o_id'])
                if res:
                    print('order found')
                    ses = request.env['pos.points.line'].update_existing_lines(req_arg['data'])
                    return "queuelines updated"
                else:
                    return "order not found"
            else:
                return "Invalid Parameter !"
